# Security baseline — capability spec

> **Baseline.** Derived from `docs/requirements.md` §6.12 (REQ-SEC-\*). Unlike every other
> capability here, these requirements are not transcribed from the vendor's published test
> cases or API list — the vendor publishes no security contract. They are derived from the
> **OWASP Top 10 (2021)** and the **OWASP API Security Top 10 (2023)**, restricted to what a
> black-box functional test can observe, and each names the category it maps to.
>
> Scope boundary: this capability covers the site's cross-cutting security behaviour —
> transport, response headers, cookies, session lifetime, cross-origin exposure, injection
> handling, credential exposure, and authorisation of personal data and state-changing
> routes. The one security control that predates it, anti-enumeration at the login form,
> stays in `authentication` as REQ-AUT-03.

## Purpose

Define what automationexercise.com must do to be safe to use, in the narrow sense a
functional test can measure: refuse the wrong caller, refuse the wrong input, and disclose
nothing it was not asked for.

Categories that are **not** observable from outside are deliberately absent rather than
silently skipped — cryptographic strength (A02 internals), vulnerable components (A06),
logging and monitoring (A09), supply-chain integrity (A08) and volumetric rate limiting
(A04/API4). `docs/requirements.md` §2.2 records each with the tooling that owns it. Two more
are **not applicable** to this site: SSRF (A10/API7), because no published endpoint accepts a
URL or host, and broken function-level authorisation (API5), because the site publishes no
roles and therefore no privileged function.

Seven of these requirements are currently **violated by the site**. Each is recorded in
`docs/requirements.md` §10 as defects D4 … D10, with the test that proves it written, run,
observed failing, and commented out in place with its defect id. The requirement states what
the site should do; the defect states what it does.
